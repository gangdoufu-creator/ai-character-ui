...
const extractEmbeddedWorkflow = async (imageUrl) => {
  ...
  while (offset < data.length) {
    ...
    if (type === 'tEXt') {
      ...
      if (nullPos !== -1) {
        ...
        const value = new TextDecoder().decode(chunkData.slice(nullPos + 1));
      ...
      if (nullPos !== -1) {
        ...
        if (compressionMethod === 0) {
          try {
            ...
            console.error(`Failed to decompress zTXt chunk for key: ${key}`, e);
          }
        } else {
          console.warn(`Unsupported zTXt compression method for key: ${key}`);
        }
      }
    }

    offset += length + 12;  // Chunk length + type + CRC
  }
  ...
  if (workflowKey) {
    try {
      const workflowJSON = JSON.parse(textChunks[workflowKey]);
      console.log(`📦 Found Workflow Data under key: ${workflowKey}`, workflowJSON);
      return workflowJSON;
    } catch (e) {
      console.error("❌ Failed to parse workflow JSON:", e);
    }
  } else {
    console.warn("❌ No workflow metadata key found in PNG.");
  }
